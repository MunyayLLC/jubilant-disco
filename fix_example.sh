...
# Example script that demonstrates the corrected approach for tilde comparison
# This implements the solution suggested in issue #33
# This function avoids the problematic tilde (~) comparison pattern

# Function that validates if a directory is safe to use (CORRECTED VERSION)
# Rejects dangerous directories like /, ~, ., .. and empty strings
# This function uses $HOME instead of "~" for proper home directory detection
validate_target_directory() {
    ...
    # CORRECTED CODE - Using $HOME instead of "~" for proper home directory detection:
